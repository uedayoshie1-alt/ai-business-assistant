/**
 * BizAssist AI - 請求書自動生成 GAS スクリプト
 *
 * 【セットアップ手順】
 * 1. Google Apps Script (script.google.com) で新しいプロジェクトを作成
 * 2. このコードを貼り付けて保存
 * 3. FOLDER_ID を請求書を保存したいGoogleドライブのフォルダIDに変更
 * 4. TEMPLATE_SHEET_ID を請求書テンプレートのスプレッドシートIDに変更
...
// 設定（ここを変更してください）
// ==============================
const FOLDER_ID = 'YOUR_GOOGLE_DRIVE_FOLDER_ID' // 請求書を保存するフォルダのID
...
function doPost(e) {
  try {
    ...
    const ssName = `請求書_${info.invoiceNumber}_${info.invoiceTo || '宛先未設定'}`
...
function buildInvoiceSheet(sheet, info, items, subtotal, tax, total) {
  ...
  // --- 請求書番号・日付 ---
  sheet.getRange('E1').setValue('請求番号').setFontSize(9).setFontColor('#666666')
  ...
  // --- 合計金額ボックス ---
  sheet.getRange('B9:E9').merge().setValue(`ご請求金額：${fmt(total)}`).setFontSize(14).setFontWeight('bold')
