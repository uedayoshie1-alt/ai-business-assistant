...
[
  ...
  const rssUrlMap = rssItems.map(it => ({ title: it.title, url: it.link }))
  ...
  const alerts = JSON.parse(jsonMatch[0]).map((alert: any) => {
    ...
    if (isGenericUrl && rssUrlMap.length > 0) {
      // アラートタイトルのキーワードでRSS URLをマッチング
      const keywords = alert.title.split(/[・、。\s]/).filter((k: string) => k.length > 2)
      const matched = rssUrlMap.find(r =>
        keywords.some((kw: string) => r.title.includes(kw))
      )
      if (matched?.url) {
        alert.sourceUrl = matched.url
      } else if (rssUrlMap[0]?.url) {
        // マッチしない場合は最新のRSS URLを使用
        alert.sourceUrl = rssUrlMap[0].url
      }
    }
    return alert
  })
